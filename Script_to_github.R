...
df$direction <- ifelse(df$avg_log2FC>0,'U','D')
...
p=plot_cell_trajectory(cds_obj,color_by = "spp")+
  ...
  scale_color_manual(values= c('#9eb3d8','#af4212') )
...
ggplot(plotdf, aes(x=Pseudotime,y=state,fill=age))+
  ...
  scale_y_discrete("")+
  scale_fill_manual(values=c('#9eb3d8','#af4212'))+
  theme_minimal()+ 
...
tmp1=plot_genes_branched_heatmap(cds_obj[row.names(subset(BEAM_res,qval<1e-4)),],
                                 ...
                                 branch_colors = c("#979797", "#9eb3d8", "#af4212"), 
